...
buildRedeemAdaFromPlutusScript = async () => {
    ...
    coasync function getWalletBalance() {
        try {
          // Try to get the wallet object that the user is selecting
          const walletObject = window.cardano && window.cardano.lace;
          ...
          // Check for Lace API availability
          ...
          // Convert balance from CBOR hex to a readable format (if needed)
          // (Implementation depends on CBOR handling library and expected format)
          const balance = convertBalanceFromCBORHex(balanceCBORHex);
        ...
        // Example balance conversion function (replace with your actual implementation)
        function convertBalanceFromCBORHex(balanceCBORHex) {
          // Example using a CBOR library (replace with your preferred library)
          const decodedBalance = CBOR.decode(balanceCBORHex);
          return decodedBalance.value; // Assuming balance is a number
        }
        ...
        };nst baseAddress = BaseAddress.from_address(shelleyChangeAddress)
